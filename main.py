...
def main():
    ...
    serial_port_label.grid(row=2, column=0, padx=(10, 0), pady=10, sticky='w')
    ...
    serial_port_combo.grid(row=2, column=1, padx=(0, 10), pady=10, sticky='w')
    ...
    clean_eeprom_button.grid(row=3, column=0, columnspan=2, padx=(10, 60), pady=10, sticky='w')
    ...
    write_font_button.grid(row=3, column=1, padx=(60, 10), pady=10, sticky='w')

    textbox = tk.Text(window, width=40, height=10)
    textbox.grid(row=4, column=0, columnspan=26, padx=10, pady=10, sticky='w')
    sys.stdout = TextRedirector(textbox)

    # 创建进度条
    progress = ttk.Progressbar(window, orient='horizontal', length=260, mode='determinate')
    # 放置进度条在窗口底部
    progress.grid(row=5, column=0, columnspan=26, padx=10, pady=10, sticky='ew')
